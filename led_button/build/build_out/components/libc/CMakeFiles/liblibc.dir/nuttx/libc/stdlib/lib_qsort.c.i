# 1 "/home/durga/QCCSDK-QCC74x/components/libc/nuttx/libc/stdlib/lib_qsort.c"
# 1 "/home/durga/QCCSDK-QCC74x/examples/peripherals/gpio/gpio_input_output/build/build_out/components/libc//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/home/durga/QCCSDK-QCC74x/components/libc/nuttx/libc/stdlib/lib_qsort.c"
# 47 "/home/durga/QCCSDK-QCC74x/components/libc/nuttx/libc/stdlib/lib_qsort.c"
...
# 48 "/home/durga/QCCSDK-QCC74x/components/libc/nuttx/libc/stdlib/lib_qsort.c" 2
# 92 "/home/durga/QCCSDK-QCC74x/components/libc/nuttx/libc/stdlib/lib_qsort.c"
...
# 165 "/home/durga/QCCSDK-QCC74x/components/libc/nuttx/libc/stdlib/lib_qsort.c"
...
loop:
  swaptype = (( char *)base - ( char *)0) % sizeof(long) || width % sizeof(long) ? 2 : width == sizeof(long)? 0 : 1;;
  swap_cnt = 0;
      ...
      for (pm = ( char *)base + width;
           ...
           pm += width)
        {
          for (pl = pm;
               ...
               pl -= width)
            {
              if (swaptype == 0) { long t = *(long *)(pl); *(long *)(pl) = *(long *)(pl - width); *(long *)(pl - width) = t; } else { swapfunc(pl, pl - width, width, swaptype); };
            }
        }
  ...
  if (swaptype == 0) { long t = *(long *)(base); *(long *)(base) = *(long *)(pm); *(long *)(pm) = t; } else { swapfunc(base, pm, width, swaptype); };
  ...
  for (; ; )
    {
      while (pb <= pc && (r = compar(pb, base)) <= 0)
        {
          if (r == 0)
            {
              swap_cnt = 1;
              if (swaptype == 0) { long t = *(long *)(pa); *(long *)(pa) = *(long *)(pb); *(long *)(pb) = t; } else { swapfunc(pa, pb, width, swaptype); };
              pa += width;
            }
      ...
      while (pb <= pc && (r = compar(pc, base)) >= 0)
        {
          if (r == 0)
            {
              swap_cnt = 1;
              if (swaptype == 0) { long t = *(long *)(pc); *(long *)(pc) = *(long *)(pd); *(long *)(pd) = t; } else { swapfunc(pc, pd, width, swaptype); };
              pd -= width;
            }
      ...
      if (swaptype == 0) { long t = *(long *)(pb); *(long *)(pb) = *(long *)(pc); *(long *)(pc) = t; } else { swapfunc(pb, pc, width, swaptype); };
      swap_cnt = 1;
      pb += width;
      pc -= width;
    }
  ...
  r = (pa - ( char *)base) < (pb - pa) ? pa - ( char *)base : pb - pa;
  if ((r) > 0) swapfunc(base, pb - r, r, swaptype);

  r = (pd - pc) < (pn - pd - width) ? pd - pc : pn - pd - width;
